...
Dot::Dot(bool chaser)
{
	...
	if (isChaser)
	{
		gDotTexture.setColor(66, 244, 66);
	}
	else
	{
		gDotTexture.setColor(244, 69, 244);
	}
...
void Dot::Init(SDL_Renderer *gRenderer)
{
	if (isChaser)
	{
		if (!gDotTexture.loadFromFile("Data/pacdot.png", gRenderer))
		{
			printf("Failed to load dot texture!\n");
		}
	}
	else {
		if (!gDotTexture.loadFromFile("Data/BlueGhost.png", gRenderer))
		{
			printf("Failed to load dot texture!\n");
		}
	}
